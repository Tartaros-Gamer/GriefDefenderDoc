...
Claim - `/claimoption <option> <value> [context[key=value]]`  
Player - `/cop <player> <option> <value> [context[key=value]]`  
Group - `/cog <group> <option> <value> [context[key=value]]`  
...
```griefdefender.economy-block-sell-return``` | 0.0 | The return ratio for selling claim blocks. Note: If set to 0 or less, the sell block feature will be disabled.
...
```griefdefender.max-accrued-blocks```   |   80000       | The limit on accrued blocks (over time). doesn't limit purchased or admin-gifted blocks. Note: If using 'wilderness-cuboids', this value is 20480000 by default.
